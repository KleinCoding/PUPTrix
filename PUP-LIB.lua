...
local hud_settings = {
    ...
    text  = {
        ...
        red = 255, green = 255, blue = 255, alpha = HUD_TEXT_ALPHA
    },
}
local hud = nil
...
local function hud_update()
    ...
    if current_state.hudView == 'SetsOnly' then
		...
		elseif current_state.hudView == 'Condensed' then
			...
			table.insert(lines, align_label("PetMatrixLayer", fmt_count(current_state.petMatrixLayer, DynamicLists.PetMatrixLayers)))
        ...
        -- Matrix name
        local matrix_idx = 1
        for i,v in ipairs(DynamicLists.Matrices) do if v == current_state.matrixName then matrix_idx = i; break end end
        table.insert(lines, align_label("Matrix", string.format("%s (%d/%d)", current_state.matrixName or "?", matrix_idx, #DynamicLists.Matrices)))
...
local function build_matrices()
    ...
    local found = false
    for _,v in ipairs(list) do if v == current_state.matrixName then found = true break end end
...
local function build_layers()
    ...
    local valid = false
    for _,v in ipairs(DynamicLists.Layers) do if v == current_state.layer then valid = true break end end
...
local function build_pet_matrix_lists()
    ...
    local ok = false
    for _,v in ipairs(layers) do if v == current_state.petMatrixLayer then ok = true break end end
...
local function update_pet_matrix_layers_for_combo(combo)
    ...
    DynamicLists.PetMatrixLayers = layers
    local found = false
    for _,v in ipairs(layers) do if v == current_state.petMatrixLayer then found = true break end end
...
local function build_custom_layers()
    ...
    -- Validate current
    local ok=false
    for _,v in ipairs(list) do if v == current_state.customLayer then ok=true break end end
...
local function resolve_gear(state)
    ...
    if gm then
        ...
        if state.playerStatus == 'Engaged' and state.petStatus == 'Engaged' then
            set = combine_safe(set, safe_get(gm, zoneGroup, 'masterPet', layer) or safe_get(gm, zoneGroup, 'masterPet', 'Normal'))
            name = ':'..state.matrixName..'.'..zoneGroup..'.masterPet.'..layer
        elseif state.playerStatus == 'Engaged' then
            set = combine_safe(set, safe_get(gm, zoneGroup, 'master', layer) or safe_get(gm, zoneGroup, 'master', 'Normal'))
    ...
    if pm and state.petMatrixCombo and state.petMatrixCombo ~= 'None' then
        ...
        if pmSet then
            ...
            name = name .. '+petMatrix.'..zoneGroup..'.'..combo..'.'..(pmSet == safe_get(pm, zoneGroup, combo, layer) and layer or 'Normal')
...
local function cycle(field, list)
    ...
    local idx = 1
    for i,v in ipairs(list) do if v==current_state[field] then idx=i break end end
...
function self_command(cmd)
    ...
    if c == 'cycle' then
		...
		elseif which == 'autorepair' then
			...
			local idx = 1
			for i,v in ipairs(thresholds) do if v == cur then idx = i break end end
